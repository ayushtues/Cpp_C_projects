...
{   // Calculate the global thread ID
    ...
    for(int i = tid; i < N; i += (gridDim.x * blockDim.x)) // the  i+= thing is for the case when we have less threads than the total number of elements, so a single thread has to do more work
    {
